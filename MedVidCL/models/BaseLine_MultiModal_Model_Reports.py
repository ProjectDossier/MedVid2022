...
parser.add_argument(
    "--i3d_feature_dir",
    type=str,
    default="../features/I3D/",
    help="Directory where I3D-extracted video features are located",
)
parser.add_argument(
    "--vit_feature_dir",
    type=str,
    default="../features/ViT/",
    help="Directory where ViT-extracted video features are located",
)
parser.add_argument(
    "--seed", type=int, default=42, help="Seed to use to initialize random numbers"
)
parser.add_argument(
    "--batch_size", type=int, default=16, help="Batch Size to train the models with"
)
parser.add_argument(
    "--epochs",
    type=int,
    default=50,
    help="Number of iterations to spend training the model",
)
parser.add_argument(
    "--lstm_learning_rate",
    type=float,
    default=4e-4,
    help="Learning Rate for LSTM Model",
)
parser.add_argument(
...
def import_datasets(vit_datatype=True):
    ...
    json_filenames = [
        pos_json
        for pos_json in os.listdir(args.source_dir)
        if pos_json.endswith(".json")
    ]
    for json_filename in json_filenames:
        datasets[json_filename] = pd.read_json(args.source_dir + "/" + json_filename)
        # Rename columns
        datasets[json_filename] = datasets[json_filename].rename(
            columns={
        ...
        datasets[json_filename]["labels"] = datasets[json_filename]["labels"].apply(
            label_vid
        )
        # Remove duplicates
        datasets[json_filename].drop_duplicates(
            subset=["text"], inplace=True, ignore_index=True
        )
        # Send to CSV
        datasets[json_filename][["labels", "text"]].to_csv(
            args.source_dir + json_filename[:-5] + ".csv", index=False
        )
        # Add feature numpy array
        if vit_datatype:
            datasets[json_filename]["features"] = datasets[json_filename][
                "YouTube_ID"
            ].apply(
                lambda x: np.load(
                    args.vit_feature_dir + json_filename[:-5] + "/" + x + ".npy"
                )
            )
        else:
            datasets[json_filename]["features"] = datasets[json_filename][
                ...
                lambda x: np.load(
                    args.i3d_feature_dir + json_filename[:-5] + "/" + x + ".npy"
                )
            )
        # Convert all numpy arrays to float32
        datasets[json_filename]["features"] = datasets[json_filename]["features"].apply(
            lambda x: x.astype("float32")
        )
        # Convert all numpy arrays to Pytorch tensors
        datasets[json_filename]["torch_features"] = datasets[json_filename][
            "features"
        ].apply(lambda x: torch.Tensor(x))
        # Change each feature column to a list
        torch_features[json_filename] = torch.nn.utils.rnn.pad_sequence(
            datasets[json_filename]["torch_features"].to_list(),
            batch_first=True,
            padding_value=0,
        )
...
for json_filename in json_filenames:
    ...
    tensor_datasets[json_filename] = TensorDataset(
        torch_features[json_filename],
        torch.from_numpy(datasets[json_filename]["labels"].values),
    )
    dls[json_filename] = DataLoader(
        tensor_datasets[json_filename],
        shuffle=False,
    ...
    vit_tensor_datasets[json_filename] = TensorDataset(
        vit_torch_features[json_filename],
        torch.from_numpy(vit_datasets[json_filename]["labels"].values),
    )
    vit_dls[json_filename] = DataLoader(
        vit_tensor_datasets[json_filename],
        shuffle=False,
        batch_size=args.batch_size,
        drop_last=True,
    )
    ...
    vit_transformer_datasets[json_filename] = (
        vit_torch_features[json_filename].detach().numpy()
    )
...
train_iter = BucketIterator(
    train, batch_size=args.batch_size, device=device, shuffle=False
)
valid_iter = BucketIterator(
    valid, batch_size=args.batch_size, device=device, shuffle=False
)
test_iter = BucketIterator(
    test, batch_size=args.batch_size, device=device, shuffle=False
)
...
tf_train_docs = list(
    zip(datasets["train.json"]["text"], datasets["train.json"]["labels"])
)
...
## pad sequences
train_texts_to_int_pad = keras.preprocessing.sequence.pad_sequences(
    train_texts_to_int, maxlen=maxlen, truncating="post", padding="post"
)
...
def train_and_test_lstm(
    ...
    for i in range(epochs):
        ...
        for (inputs, labels), ex in zip(dataset["train.json"], text_dataset["train"]):
            ...
            text = ex.text[0]
            text_len = ex.text[1]
            inputs, labels, text_labels, text, text_len = (
                inputs.to(device),
                labels.to(device),
                text_labels.to(device),
                text.to(device),
                text_len.to(device),
            )
            model.zero_grad()
    ...
    test_report = classification_report(
        y_true=test_targets, y_pred=test_preds, labels=[0, 1, 2], digits=4
    )
...
def get_compiled_model(
    max_seq_length, num_features, vocab_size, maxlen, learning_rate, dense_dim=12
):
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate, beta_2=0.999),
...
def train_and_test_transformer(
    ...
    model = get_compiled_model(
        max_seq_length, num_features, vocab_size, maxlen, learning_rate, dense_dim
    )
    history = model.fit(
        [datasets["train.json"], train_texts_to_int_pad],
        labels["train.json"],
        validation_data=(
            [datasets["val.json"], val_texts_to_int_pad],
            labels["val.json"],
        ),
        epochs=epochs,
    ...
    # Test on Set
    test_preds = model.predict([datasets["test.json"], test_texts_to_int_pad]).argmax(
        axis=-1
    )
    test_report = classification_report(
        y_true=labels["test.json"], y_pred=test_preds, labels=[0, 1, 2], digits=4
    )
    print("Test Set Classification Report:\n", test_report)
...
train_and_test_lstm(
    ...
    text_dls,
    text_field,
    "./multimodal_tmp/lstm_i3d_trained_state_dict.pt",
    I3D_MAX_SEQ_LENGTH,
    I3D_NUM_FEATURES,
    args.batch_size,
...
train_and_test_lstm(
    ...
    text_dls,
    text_field,
    "./multimodal_tmp/lstm_vit_trained_state_dict.pt",
    VIT_MAX_SEQ_LENGTH,
    VIT_NUM_FEATURES,
    args.batch_size,
    args.lstm_learning_rate,
...
trained_model = train_and_test_transformer(
    transformer_datasets,
    transformer_labels,
    "./multimodal_tmp/transformer_i3d_video_classifier",
    I3D_MAX_SEQ_LENGTH,
    I3D_NUM_FEATURES,
    vocab_size,
    maxlen,
...
vit_trained_model = train_and_test_transformer(
    vit_transformer_datasets,
    vit_transformer_labels,
    "./multimodal_tmp/transformer_vit_video_classifier",
    VIT_MAX_SEQ_LENGTH,
    VIT_NUM_FEATURES,
    vocab_size,
    maxlen,
