...
def caption_extraction(video_id):
    ...
    try:
        ...
        srt = YouTubeTranscriptApi.get_transcript(
            video_id, cookies="youtube.com_cookies.txt"
        )
    ...
    # When no manually-created or automatically-created english transcript, searches available transcripts and tries to translate them to English
        ...
        try:
            ...
            for transcript in transcript_list:
                ...
                if (
                    not {"language": "English", "language_code": "en"}
        ...
        # Log any Errors
            ...
            print(
                "YouTube UID: " + str(video_id), file=f
            )  # Print links to any Youtube video of which captions were not extracted
            print(e, file=f)
            pass
    ...
    try:
        ...
        if res["requested_subtitles"] and res["requested_subtitles"]["en"]:
            ...
            response = requests.get(
                res["requested_subtitles"]["en"]["url"], stream=True
            )
            new = re.sub(
...
def caption_extraction_v3(video_id):
    ...
    try:
        html = urlopen(
            "https://www.youtube.com/api/timedtext?&v=" + str(video_id) + "&lang=en"
        )
        ...
        full_text += " ".join(
            map(
                str,
                [
                    captions.text.replace("&#39;", "'")
                    for captions in soup.find_all("text")
                ],
            )
        )
        return full_text
