...
class MedVidQAProcessor:
    ...
    def process_data(self, data, scope):
        results = []
        for index, data_item in tqdm(
            enumerate(data), total=len(data), desc="process medvidqa {}".format(scope)
        ):
            if len(data_item) == 0:
...
def gen_or_load_dataset(configs):
    ...
    if configs.suffix is None and configs.task == "medvidqa":
        save_path = os.path.join(
            configs.save_dir,
            "_".join([configs.task, str(configs.max_pos_len)]) + ".pkl",
        )
    else:
    ...
    test_set = (
        ...
        if test_data is None
            ...
            test_data, vfeat_lens, word_dict, char_dict, configs.max_pos_len, "test"
        )
    )
    # save dataset
...
def gen_or_load_eval_dataset(configs):
    ...
    if configs.suffix is None:
        ...
        train_data_save_path = os.path.join(
            configs.save_dir,
            "_".join([configs.task, str(configs.max_pos_len), configs.suffix]) + ".pkl",
        )
